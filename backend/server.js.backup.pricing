...
app.post('/api/stripe/create-checkout-session', async (req, res) => {
  try {
    ...
    const { planType, credits } = req.body;
    
    // Define pricing based on plan type with correct credits display
    const prices = {
      basic: { amount: 999, name: `Basic Plan - ${credits || 10} Credits`, credits: credits || 10 },
      blogger: { amount: 4999, name: `Blogger Plan - ${credits || 100} Credits`, credits: credits || 100 },
      pro: { amount: 9999, name: `Pro/Agency Plan - ${credits || 240} Credits`, credits: credits || 240 }
    };
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: {
            name: prices[planType].name,
            description: `${prices[planType].credits} keyword analysis credits`,
          },
          unit_amount: prices[planType].amount,
        },
        quantity: 1,
      }],
      mode: 'payment',
...
// Verify payment session - primary endpoint
...
// Backward compatibility alias for verify-payment
...
app.post('/api/stripe/create-access-key', async (req, res) => {
  try {
    const { sessionId, planType, credits, amountPaid } = req.body;
    ...
    const existingKey = database.db.prepare('SELECT * FROM payment_keys WHERE session_id = ?').get(sessionId);
    if (existingKey) {
      return res.json({
        success: true,
        accessKey: existingKey.access_key,
        message: 'Access key already created for this payment'
      });
    }
    ...
    // Map plan types to valid database values
    const planMapping = {
      'basic': 'basic',
      'blogger': 'blogger', 
      'pro': 'pro'
    ...
    const dbPlan = planMapping[planType] || 'basic';
    
    // Create the access key
    await database.createAccessKey(accessKey, dbPlan, credits, session.customer_details?.email);
    
    // Record the payment-key association
    database.db.prepare(`
      INSERT OR IGNORE INTO payment_keys (session_id, access_key, plan_type, credits, amount_paid, created_at)
      VALUES (?, ?, ?, ?, ?, datetime('now'))
    `).run(sessionId, accessKey, planType, credits, amountPaid);
    
    console.log(`Created access key ${accessKey} for payment session ${sessionId}`);
    ...
    console.error('Error creating access key from payment:', error);
    res.status(500).json({ error: 'Failed to create access key from payment' });
  }
});
